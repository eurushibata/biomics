...
download <- function(...){
	## Entre de id from function biOMICS. Man can choose them
	l <- list(...)
	l <- unlist(l)
	l <- paste0(l, collapse=",")
	...
	sql <- paste0("SELECT name, path, size, checksum FROM File where Experiment_Sample_id in (", l, ")")
	#print(sql)
	...
	for(i in seq(1:nrow(qs))){
		#print(qs[i,])
		
		f <- CFILE(qs[i,]$name, mode="wb")
		curlPerform(url=qs[i,]$path, writedata=f@ref, curl=handle, header=TRUE)
		close(f)
...
biOMICS <- function(keyword, exact=FALSE) {
	...
	ontology.table.local <- subset(database, database$term_id %in% as.vector(ontology.table.remote$term.id))
	...
	if (nrow(ontology.table.local)==0) {
		...
		t.encode <- nrow(subset(out, project == "encode"))
		t.roadmap <- nrow(subset(out, project == "roadmap"))
		t.tcga <- nrow(subset(out, project == "tcga"))
